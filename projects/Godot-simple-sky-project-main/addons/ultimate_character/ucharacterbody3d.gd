...
## How long will the camera be frozen at the start of the uni reset
@export var CUTSCENE_LENGHT : float = 3.5
...
var scene
@export var can_uni_reset = false
var in_cutscene : bool
signal sliding
signal in_cutscene_signal
...
func _input(event):
	if !Engine.is_editor_hint():
		if event is InputEventMouseMotion and !is_sliding and !in_cutscene:
...
func _physics_process(delta):
	if !Engine.is_editor_hint():
		...
		if Input.is_action_pressed(CROUCH) or is_sliding:
			...
			# Handle sliding
			if is_sprinting and is_on_floor() and input_dir != Vector2.ZERO:
				is_sliding = true
		...
		if is_sliding:
			emit_signal("sliding", true)
			slide_timer -= delta
			can_uni_reset = false
			if slide_timer <= 0:
				emit_signal("sliding", false)
				can_uni_reset = true
				is_sliding = false
		...
		if is_on_floor() and !is_sliding and input_dir != Vector2.ZERO and !in_cutscene:
	...
	if in_cutscene:
		head_node.rotation.x = lerpf(head_node.rotation.x, 0, delta * 8.0)


func _on_input_handler_uni_reset_signal() -> void:
	emit_signal("in_cutscene_signal", true)
	in_cutscene = true
	await get_tree().create_timer(CUTSCENE_LENGHT).timeout
	emit_signal("in_cutscene_signal", false)
	in_cutscene = false
